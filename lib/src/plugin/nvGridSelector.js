...
nvui.controls.GridSelector.prototype._initTbBody = function() {
	...
	nvui.data.getJSON("controls/grid-selector/config.mvc", {
		...
		th.width(24);
		th.find(".nv-gridselector-hd-check").click(function() {
...
nvui.controls.GridSelector.prototype._initPageInfo = function(pageInfo) {
	...
	that.pageCount = pageInfo["pageCount"];
	that.recordCount = pageInfo["recordCount"];

	that.pager.pagination(pageInfo["recordCount"], {
		callback: $.proxy(that._cbGetData, that),
		prev_text: '上一页',
		next_text: '下一页',
		items_per_page: that.options.pageRows,
		num_display_entries: 6,
		current_page: pageInfo["currPage"] - 1,
		num_edge_entries: 2
	});

	var info = pageInfo["currPage"] + "/" + that.pageCount + " 共 " + that.recordCount + " 条记录";
	that.rec.html(info);
};
...
nvui.controls.GridSelector.prototype._initSelectBox = function() {
	...
	var headHs = $(".nv-gridselector-hd-check");
	cellHs.each(function() {
		var $this = $(this);
