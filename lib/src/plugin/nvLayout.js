 ...
 */
(function($) {
    $.fn.nvLayout = function(options) {
        return nvui.run.call(this, "nvuiLayout", arguments);
    };

    $.fn.nvuiGetLayoutManager = function() {
        return nvui.run.call(this, "nvuiGetLayoutManager", arguments);
    };
    ...
    nvui.controls.Layout = function(element, options) {
        nvui.controls.Layout.base.constructor.call(this, element, options);
    };
    nvui.inherits(nvui.controls.Layout, nvui.core.UIComponent, {
        __getType: function() {
            return "nvui.controls.Layout";
        },
        __idPrev: function() {
            return 'Layout';
        },
        ...
        _render: function() {
            ...
            g.layout.addClass("nv-layout");
            g.width = g.layout.width();
            //top
            if ($("> div[position=top]", g.layout).length) {
                g.top = $("> div[position=top]", g.layout).wrap('<div class="nv-layout-top"></div>').parent();
                g.top.content = $("> div[position=top]", g.top).addClass("nv-layout-content");
                g.topHeight = p.topHeight;
                g.topHeight && g.top.height(g.topHeight);
            }

            //bottom
            if ($("> div[position=bottom]", g.layout).length) {
                g.bottom = $("> div[position=bottom]", g.layout).wrap('<div class="nv-layout-bottom"></div>').parent();
                g.bottom.content = $("> div[position=bottom]", g.bottom).addClass("nv-layout-content");

                g.bottomHeight = p.bottomHeight;
                g.bottomHeight && g.bottom.height(g.bottomHeight);

                //set title
                var bottomtitle = g.bottom.content.attr("title");
                if (bottomtitle) {
                    g.bottom.header = $('<div class="nv-layout-header"></div>');
            ...
            //left
            if ($("> div[position=left]", g.layout).length) {
                g.left = $("> div[position=left]", g.layout).wrap('<div class="nv-layout-left""></div>').parent();
                g.left.content = $("> div[position=left]", g.left).addClass("nv-layout-content");

                if (g.left.content.attr("hidetitle")) {
                    g.left.content.attr("title", "");
                } else {
                    g.left.header = $('<div class="nv-layout-header"><div class="nv-layout-header-toggle"></div><div class="nv-layout-header-inner"></div></div>');
                    g.left.prepend(g.left.header);
                    g.left.header.toggle = $(".nv-layout-header-toggle", g.left.header);

                    if (!p.allowLeftCollapse) $(".nv-layout-header-toggle", g.left.header).remove();

                    //set title
                    var lefttitle = g.left.content.attr("title");
                    if (lefttitle) {
                        g.left.content.attr("title", "");
                        $(".nv-layout-header-inner", g.left.header).html(lefttitle);
                    }
                }

                g.leftWidth = p.leftWidth;
                g.leftWidth && g.left.width(g.leftWidth);
            }

            //center
            if ($("> div[position=center]", g.layout).length) {
                g.center = $("> div[position=center]", g.layout).wrap('<div class="nv-layout-center" ></div>').parent();
                g.center.content = $("> div[position=center]", g.center).addClass("nv-layout-content");
                ...
                if (g.center.content.attr("hidetitle")) {
                    ...
                    if (centertitle) {
                        g.center.content.attr("title", "");
                        g.center.header = $('<div class="nv-layout-header"></div>');
                ...
                g.centerWidth && g.center.width(g.centerWidth);

                //centerBottom
                if ($("> div[position=centerbottom]", g.layout).length) {
                    g.centerBottom = $("> div[position=centerbottom]", g.layout).wrap('<div class="nv-layout-centerbottom" ></div>').parent();
                    g.centerBottom.content = $("> div[position=centerbottom]", g.centerBottom).addClass("nv-layout-content");
                    ...
                    if (g.centerBottom.content.attr("hidetitle")) {
                        ...
                        if (centertitle) {
                            g.centerBottom.content.attr("title", "");
                            g.centerBottom.header = $('<div class="nv-layout-header"></div>');
                            g.centerBottom.prepend(g.centerBottom.header);
                            g.centerBottom.header.html(centertitle);
                        }
                    }

                    g.centerWidth && g.centerBottom.width(g.centerWidth);
                }
            }

            //right
            if ($("> div[position=right]", g.layout).length) {
                g.right = $("> div[position=right]", g.layout).wrap('<div class="nv-layout-right"></div>').parent();
                g.right.content = $("> div[position=right]", g.right).addClass("nv-layout-content");

                if (g.right.content.attr("hidetitle")) {
                    g.right.content.attr("title", "");
                } else {
                    g.right.header = $('<div class="nv-layout-header"><div class="nv-layout-header-toggle"></div><div class="nv-layout-header-inner"></div></div>');
                    g.right.prepend(g.right.header);
                    g.right.header.toggle = $(".nv-layout-header-toggle", g.right.header);
                    if (!p.allowRightCollapse) $(".nv-layout-header-toggle", g.right.header).remove();

                    //set title
                    var righttitle = g.right.content.attr("title");
                    if (righttitle) {
                        g.right.content.attr("title", "");
                        $(".nv-layout-header-inner", g.right.header).html(righttitle);
                    }
                }

                //set width
                g.rightWidth = p.rightWidth;
                g.rightWidth && g.right.width(g.rightWidth);
            }

            //lock
            g.layout.lock = $("<div class='nv-layout-lock'></div>");
            ...
            g.leftCollapse = $('<div class="nv-layout-collapse-left" style="display: none; "><div class="nv-layout-collapse-left-toggle"></div></div>');
            g.rightCollapse = $('<div class="nv-layout-collapse-right" style="display: none; "><div class="nv-layout-collapse-right-toggle"></div></div>');
            g.layout.append(g.leftCollapse).append(g.rightCollapse);
            g.leftCollapse.toggle = $("> .nv-layout-collapse-left-toggle", g.leftCollapse);
            g.rightCollapse.toggle = $("> .nv-layout-collapse-right-toggle", g.rightCollapse);
            g._setCollapse();
            //init
            g._bulid();
        ...
        _bulid: function() {
            var g = this,
                p = this.options;
            $("> .nv-layout-left .nv-layout-header,> .nv-layout-right .nv-layout-header", g.layout).hover(function() {
                $(this).addClass("nv-layout-header-over");
            }, function() {
                $(this).removeClass("nv-layout-header-over");

            });
            $(".nv-layout-header-toggle", g.layout).hover(function() {
                $(this).addClass("nv-layout-header-toggle-over");
            }, function() {
                $(this).removeClass("nv-layout-header-toggle-over");

            });
            $(".nv-layout-header-toggle", g.left).click(function() {
                g.setLeftCollapse(true);
            });
            $(".nv-layout-header-toggle", g.right).click(function() {
        ...
        _setCollapse: function() {
            ...
            g.leftCollapse.hover(function() {
                $(this).addClass("nv-layout-collapse-left-over");
            }, function() {
                $(this).removeClass("nv-layout-collapse-left-over");
            });
            g.leftCollapse.toggle.hover(function() {
                $(this).addClass("nv-layout-collapse-left-toggle-over");
            }, function() {
                $(this).removeClass("nv-layout-collapse-left-toggle-over");
            });
            g.rightCollapse.hover(function() {
                $(this).addClass("nv-layout-collapse-right-over");
            }, function() {
                $(this).removeClass("nv-layout-collapse-right-over");
            });
            g.rightCollapse.toggle.hover(function() {
                $(this).addClass("nv-layout-collapse-right-toggle-over");
            }, function() {
                $(this).removeClass("nv-layout-collapse-right-toggle-over");
            });
        ...
        _addDropHandle: function() {
            ...
            if (g.left && p.allowLeftResize) {
                g.leftDropHandle = $("<div class='nv-layout-drophandle-left'></div>");
            ...
            if (g.right && p.allowRightResize) {
                g.rightDropHandle = $("<div class='nv-layout-drophandle-right'></div>");
            ...
            if (g.top && p.allowTopResize) {
                g.topDropHandle = $("<div class='nv-layout-drophandle-top'></div>");
            ...
            if (g.bottom && p.allowBottomResize) {
                g.bottomDropHandle = $("<div class='nv-layout-drophandle-bottom'></div>");
            ...
            if (g.centerBottom && p.allowCenterBottomResize) {
                g.centerBottomDropHandle = $("<div class='nv-layout-drophandle-centerbottom'></div>");
            ...
            g.draggingxline = $("<div class='nv-layout-dragging-xline'></div>");
            g.draggingyline = $("<div class='nv-layout-dragging-yline'></div>");
            g.mask = $("<div class='nv-dragging-mask'></div>");
        ...
        _updateCenterBottom: function(isHeightResize) {
            ...
            if (g.centerBottom) {
                if (isHeightResize) {
                    ...
                    g.centerBottom.css({
                        top: centerTop + centerHeight - centerBottomHeight + p.space - 3
                    });
                    g.center.height(centerHeight - centerBottomHeight - 2 - p.space);
                }
        ...
        _start: function(dragtype, e) {
            ...
            if (dragtype == 'leftresize' || dragtype == 'rightresize') {
                ...
                g.mask.height(g.layout.height()).removeClass("nv-layout-ymask").addClass("nv-layout-xmask").show();
                ...
                g.mask.height(g.layout.height()).removeClass("nv-layout-xmask").addClass("nv-layout-ymask").show();
            ...
            g.layout.lock.show();
            if ($.support.msie || $.support.safari) $('body').bind('selectstart', function() {
                return false;
            }); // 不能选择

            $(document).bind('mouseup', $.proxy(g._stop, g));
            $(document).bind('mousemove', $.proxy(g._drag, g));
        },
        _drag: function(e) {
            var g = this,
                p = this.options;
        ...
        _stop: function(e) {
            ...
            if (g.xresize && g.xresize.diff != undefined) {
                ...
                if (g.dragtype == 'topresize') {
                    ...
                    g._updateCenterBottom(true);
                    g._onResize();
                    $(document).trigger("EVENT_LAYOUT_BOTTOM_RESIZE");
            ...
            g.layout.lock.hide();
            if ($.support.msie || $.support.safari)
                $('body').unbind('selectstart');
            $(document).unbind('mousemove', $.proxy(g._drag, g));
            $(document).unbind('mouseup', $.proxy(g._stop, g));
            $('body').css('cursor', '');
        }
    });
})(jQuery);